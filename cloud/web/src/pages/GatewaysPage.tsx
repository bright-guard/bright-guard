            ...
            {gateways.map((g) => {
              ...
              return (
                <tr key={g.id} className="hover:bg-slate-900/40">
                  ...
                  <td className="px-4 py-3">
                    <Link to={`/app/gateways/${g.id}`} className="text-brand-300 hover:underline">
                      {g.name}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-slate-400">{relativeTime(g.lastSeenAt)}</td>
      ...
      >
        {!result ? (
          ...
          <>
            ...
            <div className="mt-4 rounded-md border border-amber-700/60 bg-amber-950/40 px-3 py-2 text-xs text-amber-200">
              This token will not be shown again. Copy it now.
            </div>
            <pre className="mt-4 overflow-x-auto whitespace-pre-wrap break-all rounded-md border border-slate-700 bg-slate-950 p-3 text-xs text-slate-200">
